...
your application. Steps performed by
...
pass it to
[`rhino::app()`](https://appsilon.github.io/rhino/reference/app.md) as
arguments as we would with a normal `shiny::shinyApp() call`?

The reasoning behind this structure is to enforce consistent use of the
[box](https://klmr.me/box/) modules throughout the application. A file
loaded with [`box::use()`](https://klmr.me/box/reference/use.html) can
only load other modules/packages with
[`box::use()`](https://klmr.me/box/reference/use.html). In short, this
means that we cannot use the
[`library()`](https://rdrr.io/r/base/library.html) or
[`source()`](https://rdrr.io/r/base/source.html) functions in our app.
This is an important distinction from traditional Shiny structure, where
we are simply sourcing `app.R` when the app is loaded.
