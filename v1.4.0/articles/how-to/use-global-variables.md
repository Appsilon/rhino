...
Rhino uses
[`box`](https://appsilon.github.io/rhino/articles/explanation/box-modules.html)
...
Most of the time, it is sufficient to define a constant and then export
and use it throughout the app.

However, there are instances when an object may need to change its value
as the app runs. In such cases, Rhino suggests the following ways to
handle global variables.

Note: Using global variables can make the app more difficult to manage
and understand. If the variable can change its value at any point in the
app, future changes to the app must consider how this global variable
will be affected. A changing global variable may also make testing
difficult because tests may affect the global variables. The tests are
...
In `R`, [global variables live inside
`.GlobalEnv`](http://adv-r.had.co.nz/Environments.md). Global variables
can be updated within a function using `<<-`.
...
When code is loaded with
[`box::use()`](https://klmr.me/box/reference/use.html), global variables
live inside the [module’s own immutable
environment](https://klmr.me/box/articles/mod-env-hierarchy.html#environments-1).
Updating global variables with `<<-` will not work.
...
To overcome `box`’s feature of limiting scope, Rhino suggests creating a
new environment and use that environment to contain the global
variables.
...
Alternatively, variables can still be stored in and imported from
`.GlobalEnv`. The variable must also be defined and updated using `<-`.
...
Rhino suggests using arguments in module servers for explicit handling
of session variables or user inputs.
...
However, `shiny` has support for
[`session$userData`](https://shiny.rstudio.com/reference/shiny/latest/session.html),
