...
## Minimal `app.R`
Shiny sources `app.R` the traditional way, so `library()` and `source()` could be used.
Box modules can only load other box modules,
so by keeping the `app.R` minimal and loading the whole app with box
we can be confident that it is properly used throughout the app.

Additionally, `rhino:app()` gives us full control over app startup. It does:
1. Purge box cache (reload app without restarting R)
2. Configure logger (level, file)
3. Configure static files
4. Load main module / legacy entrypoint
...
Introducing box to existing apps written without it helped to improve structure and find bugs.
