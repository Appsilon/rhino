...
2.  `test_r` accepts additional arguments passed to
    [`testthat::test_dir`](https://testthat.r-lib.org/reference/test_dir.html).
3.  Bugfix for [`format_r()`](reference/format_r.md) and single files.
    ([\#625](https://github.com/Appsilon/rhino/issues/625))
4.  Added `auto_test_r` for automated rerunning R unit test after
    changing R files.
5.  Added `devmode` for running JS build, Sass build, R unit tests and
...
1.  Added check for `box.lsp` package in `.Rprofile` before applying the
...
See *[How-to: Rhino 1.10 Migration
Guide](https://appsilon.github.io/rhino/articles/how-to/migrate-1-10.html)*

1.  Integrated `box.lsp` for auto-complete support for `box` modules in
    VS Code.
2.  Integrated `box.linters` styling functions to style
    [`box::use()`](https://klmr.me/box/reference/use.html) calls
    according to the Rhino style guide.
3.  Added compatibility check for `treesitter` and `treesitter.r`
    dependencies
...
See *[How-to: Rhino 1.9 Migration
Guide](https://appsilon.github.io/rhino/articles/how-to/migrate-1-9.html)*

1.  Added `sass: custom` configuration option for cleaner integration
    with `bslib`.
2.  Introduced [`format_js()`](reference/format_js.md) and
    [`format_sass()`](reference/format_sass.md) powered by
    [prettier](https://prettier.io).
    - **Note:** [`lint_js()`](reference/lint_js.md) and
      [`lint_sass()`](reference/lint_sass.md) report styling errors.
      They *might* complain about formatting done with
      [`format_js()`](reference/format_js.md) and
      [`format_sass()`](reference/format_sass.md) functions; however, we
      haven’t spotted any issues so far. If you face any problems with
      this, then please [raise an issue on
      GitHub](https://github.com/Appsilon/rhino/issues/new/choose)
...
2.  Major refactor of [`rhino::app()`](reference/app.md):
...
3.  Add support for
    [`shiny.autoreload`](https://shiny.posit.co/r/reference/shiny/latest/shinyoptions.html).
...
3.  Upgrade tools based on Node.js:
    - [`test_e2e()`](reference/test_e2e.md) now uses `cypress` 13.6
    - [`build_js()`](reference/build_js.md) now uses `webpack` 5.89
    - [`build_sass()`](reference/build_sass.md) now uses `sass` 1.69
    - [`lint_js()`](reference/lint_js.md) now uses `eslint` 8.56
    - [`lint_sass()`](reference/lint_sass.md) now uses `stylelint` 14.16
      (the last major version supporting stylistic rules)
...
1.  New [`pkg_install()`](reference/dependencies.md) and
    [`pkg_remove()`](reference/dependencies.md) functions to simplify
    dependency management in Rhino.
2.  Add support for using React in Rhino (tutorial, JS function
    `registerReactComponents()`, R function
    [`react_component()`](reference/react_component.md)).
...
1.  [`test_r()`](reference/test_r.md) now clears the environment of
    loaded box modules before tests are run. This removes the need for
...
1.  Don’t use symbolic links internally. This fixes a couple of issues
    ...
    - The [`build_js()`](reference/build_js.md) and
      [`build_sass()`](reference/build_sass.md) functions now work with
      `watch = TRUE`.
    - The [`lint_js()`](reference/lint_js.md) function now works when
      imports are used in JavaScript.
2.  Drop dependency on Yarn - only Node.js is now required.
3.  Improved Rhino CI:
    - Run [`build_js()`](reference/build_js.md) and
      [`build_sass()`](reference/build_sass.md) CI.
    - Use R version from the lockfile.
    - Upgrade to `r-lib/actions/setup-r@v2`.
4.  The [`lint_r()`](reference/lint_r.md) now accepts a `paths` argument
    which can be used to run it on specific files.
5.  The [`init()`](reference/init.md) function will refuse to run in the
    home directory unless `force = TRUE` is passed.
...
2.  Sass-specific at-rules are now recognized by
    [`rhino::lint_sass()`](reference/lint_sass.md)
